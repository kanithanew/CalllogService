...
*/
    String GET_HOMEPAGE = "select a.callid,c.touser,c.fromuser,a.modulename,a.submodulename,a.payunit,a.calldate,a.callusername, a.callstatus as logprocess ,CASE    WHEN  a.helpdeskowner is null THEN 'OpenCall' WHEN  a.helpdeskOwner ='Helpdesk' and a.idopen =?1 THEN 'MyNewCall'   WHEN  a.helpdeskOwner ='Helpdesk' THEN 'NewCall'   WHEN  a.calllogin =?1 THEN 'MyInbox'  ELSE  'ListCall' END as status,c.datetimesend  from all_headercall a INNER JOIN  (select distinct(d.callid) from callpending_process d left join lasted_logprocess e on d.callid = e.callid where (e.repperson ='Helpdesk') or (d.touser=?1  or d.fromuser =?1)) b ON a.callid=b.callid INNER JOIN lasted_logprocess c on a.callid=c.callid order by a.callid";
