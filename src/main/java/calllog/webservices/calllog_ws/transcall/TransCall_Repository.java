...
public interface TransCall_Repository extends JpaRepository<TransCall, Long>, JpaSpecificationExecutor<TransCall> {
    ...
    String GET_REPORT = "select d.idlogprocess,d.datetimesend,d.namerecord,d.logprocess,e.callusername as repname from ( select a.* ,CASE WHEN touser  = 'Update' OR touser  = 'Create' OR touser  = 'Edit' OR touser  = 'Close' THEN fromuser  ELSE touser END AS repperson,b.callusername as namerecord from gfmis_logprocess a left join gfmis_staff b on a.fromuser=b.calllogin )d inner join gfmis_staff e on d.repperson=e.calllogin where d.callid =?1 ORDER BY d.idlogprocess ";
